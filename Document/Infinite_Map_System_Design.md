...
## 6. 멀티플레이 동기화 (Multiplayer Synchronization)

리슨 서버 단계에서는 서버가 맵 타일 생성의 단일 소유자입니다. `AHPlayerCharacter`는 서버 권한에서만 `UHInfiniteMapManager::UpdateMap()`을 호출하고, 서버가 생성한 `AHMapTile` 및 `AHMapProp`은 actor/movement replication을 통해 클라이언트에 전달됩니다.

현재 방식은 클라이언트가 바닥 타일을 직접 생성하지 않도록 하여 접속 클라이언트가 타일 없는 월드에서 추락하는 문제를 막습니다. 다음 단계에서는 여러 플레이어 위치를 모두 포함하는 타일 유지 범위와, 복제 액터 풀링 시 반납/숨김/충돌 상태 동기화 정책을 별도로 정리해야 합니다.
