...
### 3.1 Drag Phase (`UHSkillGemInventoryEntryUI`)
*   사용자가 인벤토리의 젬 아이콘을 드래그하면 `UHGemDragDropOp`이 생성됩니다.
*   **Payload**: 드래그 중인 `UHGemBase` 인스턴스 정보가 담깁니다.
*   **Visual**: 젬의 아이콘이 마우스 커서를 따라다니도록 `DefaultDragVisual`이 설정됩니다.

### 3.2 Drop Phase (`UHEquipGemSlot`)
*   장착 슬롯 위에 젬이 드롭되면 `NativeOnDrop`이 실행됩니다.
*   젬의 타입(Main/Support)에 따라 장착(`EquipGem`) 또는 보조 젬 연결(`AddSupportGem`) 로직이 분기됩니다.
*   성공 시 위에서 정의한 소유권 이전 로직이 실행됩니다.
