...
*   **현상**: `AHBaseCharacter`의 `CurrentHP`, `MaxHP` 및 `AHPlayerCharacter`의 `CurrentExp`, `MaxExp` 변수가 여전히 존재하며, `AttributeSet`의 값과 동시에 갱신되고 있음.
*   **위험성**: 두 값의 동기화가 깨질 경우 논리적 오류 발생 가능성이 매우 높음.
*   **해결 방안**: 구형 변수들을 완전히 제거하고, 모든 로직과 UI가 `AttributeSet`의 값을 직접 참조하도록 통합 필요.

### 1.2 AttributeSet과 Character 클래스의 강한 결합 (Tight Coupling)
*   **현상**: `HCharacterAttributeSet`이 `AHPlayerCharacter`로 캐스팅하여 `OnLevelUp()`을 직접 호출하거나, `Character->OnHPChanged`를 직접 브로드캐스트함.
...
- [ ] **구형 변수(CurrentHP, Exp 등) 제거 및 단일화**
- [ ] **AttributeSet의 캐릭터 직접 참조 제거 (이벤트 기반 전환)**
